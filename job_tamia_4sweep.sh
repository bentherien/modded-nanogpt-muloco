...
#SBATCH --job-name=muloco_4sweep
#SBATCH --output=logs/slurm_%j.out
#SBATCH --error=logs/slurm_%j.err

# ============================================================
# Run 4 independent 1-GPU experiments in parallel on Tamia (4xH100 node)
# Each GPU runs a different hyperparameter config
# Pass configs via: CONFIG1="olr,omom,H" CONFIG2=... CONFIG3=... CONFIG4=...
# ============================================================
...
run_config() {
    ...
    if [ "$USE_OUTER" = "0" ]; then
        export WANDB_RUN_NAME="tamia_baseline_gpu${GPU}"
    else
        export WANDB_RUN_NAME="tamia_olr${OLR}_omom${OMOM}_H${H}_gpu${GPU}"
    fi
